...
function BOSHConnection (opts) {
  ...
  this.cookies = opts.cookies || []; // string array
    ...
    function (err, res, body) {
      ...
      if (err) {
        if (retry < that.maxHTTPRetries) {
          ...
          that.emit('error', 'Max HTTP retries exceeded.')
          return cb(err)
        }
      }
      ...
      if (setcookie) {
        var cookies = setcookie.map(c => c.split(';')[0]);
        that.cookies = that.cookies.concat(cookies);
      }
